    ...
    try:
        def decorator(f):
            def newf(*args):
                ...
                # Dominik fix for class methods, with self param as first
                if "__main__" in str(argtypes[0]):
                    argtypes = argtypes[1:]
                assert len(argtypes) == len(types)
                # end ----
                if argtypes != types:
...
def myfunc(i1, i2, i3):
    pass
myfunc(1,0,22) # TypeWarning:  'myfunc' method accepts (int, int, float), but was given (int, int, int)
myfunc("1",0,1) # TypeWarning:  'myfunc' method accepts (int, int, float), but was given (str, int, int)
myfunc(1,1,1.33) # OK, no warnings
'''

## class example
class myClass(object):
    @accepts(int,float,str)
    def myMeth(self, v1,v2,v3):
        pass
obj = myClass()
obj.myMeth(0,1.33,"s")
